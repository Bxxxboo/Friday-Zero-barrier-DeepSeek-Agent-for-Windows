...
- 微信：PowerShell 复制到工作区根目录的 docx 等附件收不到（补发只扫 delivered/）
- 微信：`Copy-Item` 保留源文件旧时间戳导致回合末补发跳过
- 微信：重试时 delivered/ 里未发文件不再因「回合前已存在」而漏发
- 微信：助手声称「对话结束后自动发送」但未实际推送时，剥掉假承诺并提示
- 微信：用户仅问「为啥有后缀」等解释类问题时，不再误触发删改/重发文件

### 改进

- 微信多发附件：回合快照 diff + 工作区根目录扫描；`copy_file` 到 delivered/ 仍为推荐路径
- 微信已发附件默认 7 天后自动清理（无需设置页）
- 系统提示：禁止承诺「稍后会发到微信」；纯提问场景只文字解释
...
- 微信远程：生图、截图、Word/PPT、txt/md/csv/json 等任务完成后自动发图片或文件附件
- 设置 → 微信桥接：「任务完成后自动发送图片/文件到微信」开关
- 发送已有文件：复制到工作区 `.friday/delivered/` 后自动推送；支持桌面/下载等常用文件夹路径
- 超长微信文字回复附「请在电脑打开我的微信会话续读」提示

### 修复

- 微信审批卡乱码（PowerShell 格式化字符串误当路径、命令摘要被截断）
- 微信远程不再误称「无法直接发文件」，会主动用工具定位并复制文件后发送

### 改进

- 微信审批补充说明去掉命令原文，按词边界截断，避免 `-Directo` 类截断

---

## 1.3.7（2026-06-11）

**状态栏稳定、自定义模型可删与国内官网镜像**

### 新功能

- 官网 Gitee Pages 国内镜像（`bxxxboo.gitee.io/friday`），国内无需 VPN
- 设置 → 关于：「官网（国内）」指向 Gitee Pages

### 修复

- 状态栏 API/视觉/生图：测试通过后不再误报离线或反复「检测中」
- 微信 remote 有会话映射时显示「微信 在线」
- 删除自定义大模型后凭据合并不再复活已删条目
- 删除自定义模型：未选中时提示先选中目标

### 改进

- 启动连通性检测快路径，减少长时间卡在检测中
- MCP 工具列举失败短时缓存，降低日志刷屏
...
- 状态栏上下文占用百分比与 token 预算提示
- 长对话检查点（20%/45%/70%）与工作记忆 checkpoint
- 接近上限时自动折叠历史并 rebuild（计划/检查点/记忆）
- 跨会话历史 FTS 搜索与会话 fork
- 可选 Dream 记忆蒸馏；设置页上下文智能配置

### 修复

- 生图：余额不足/多端点/备用 URL/状态栏误报等
- 生图 Base URL 不再错误继承大模型地址
- 微信桥接与路径守卫加固

### 改进

- 中转 404/503 等错误提示更易读；Goal 校验减少过早收尾
...
- 打开星期五后自动检查新版本；有更新时弹出更新建议
- 弹窗支持「一键更新并重启」「手动下载」「稍后提醒」（同版本点稍后后不再重复弹出）
...
- **状态栏**：轮询不再闪「检测中」；Gateway 在线统一显示为「微信 在线」
- **多会话**：思考/生成中可切换其他会话，当前对话在后台继续
- **微信桥接**：审批卡刷屏、拒绝或超时后重复结束语
- **一键更新**：下载替换后应用可靠退出并由安装脚本带 `--install-launch` 重启
- **Node**：运行时应满足 OpenClaw 最低版本（22.19.0）
- **向导**：支持选择非 DeepSeek 服务商并切换 profile
- **视觉/生图**：切换服务商后新 Key 不再被凭据库旧值覆盖

### 改进

- 微信审批提示卡片精简
- 设置页更新完成时前端兜底 `close_window`，与后端 force exit 配合

### 测试

- 扩充：status_bar、update_installer、weixin_bridge、node_runtime、category_profiles 等

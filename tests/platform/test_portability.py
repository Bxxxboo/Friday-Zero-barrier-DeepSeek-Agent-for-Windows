...
def test_try_migrate_legacy_appdata_copies_fernet_pair(tmp_appdata, monkeypatch):
    from cryptography.fernet import Fernet

    from friday.portability import try_migrate_legacy_appdata

    appdata_root = tmp_appdata.parent
    legacy = appdata_root / "Friday-Test"
    legacy.mkdir()
    key = Fernet.generate_key()
    (legacy / ".fernet_key").write_bytes(key)
    f = Fernet(key)
    token = f.encrypt(b"sk-test-legacy-key-12345").decode("utf-8")
    payload = {
        "api_key": f"fernet:{token}",
        "llm_provider": "deepseek",
        "base_url": "https://api.deepseek.com",
        "model": "deepseek-chat",
        "settings_schema_version": 7,
    }
    (legacy / "settings.json").write_text(json.dumps(payload), encoding="utf-8")

    assert try_migrate_legacy_appdata()

    loaded = load_settings()
    assert loaded.api_key == "sk-test-legacy-key-12345"
